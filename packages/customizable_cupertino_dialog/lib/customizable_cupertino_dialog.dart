...
const double _kCornerRadius = 14;
...
const double _kDialogMinButtonHeight = 45;
const double _kDialogMinButtonFontSize = 10;
...
const double _kActionSheetEdgePadding = 8;
const double _kActionSheetCancelButtonPadding = 8;
const double _kActionSheetContentHorizontalPadding = 16;
const double _kActionSheetContentVerticalPadding = 13.5;
const double _kActionSheetActionsSectionMinHeight = 84;
const double _kActionSheetButtonHorizontalPadding = 10;
...
///
/// {@tool dartpad}
/// This sample shows how to use a [CustomizableCupertinoAlertDialog].
///	The [CustomizableCupertinoAlertDialog] shows an alert with a set of two choices
...
class CustomizableCupertinoAlertDialog extends StatefulWidget {
  ...
  const CustomizableCupertinoAlertDialog({
    super.key,
    this.title,
    this.content,
  ...
  final CupertinoDynamicColor? dialogColor;

  @override
  State<CustomizableCupertinoAlertDialog> createState() =>
      _CustomizableCupertinoAlertDialogState();
}

class _CustomizableCupertinoAlertDialogState
    extends State<CustomizableCupertinoAlertDialog> {
  ...
  Widget? _buildContent(BuildContext context) {
    final hasContent = widget.title != null || widget.content != null;
    if (!hasContent) {
      return null;
    }
    ...
    return ColoredBox(
      color: CupertinoDynamicColor.resolve(
        widget.dialogColor ?? _kDialogColor,
        context,
      ),
      child: child,
    );
    ...
    return MediaQuery.removePadding(
      ...
      child: LayoutBuilder(
        builder: (BuildContext context, BoxConstraints constraints) {
          final contentSection = _buildContent(context);
          final actionsSection = _buildActions();
...
/// in [CustomizableCupertinoAlertDialog] or rendering custom surface colors.
///
/// {@tool dartpad}
...
/// See also:
///
///  * [CustomizableCupertinoAlertDialog], which is a dialog with a title, content, and
///    actions.
///  * <https://developer.apple.com/design/human-interface-guidelines/alerts/>
class CupertinoPopupSurface extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final filter = _buildFilter(CupertinoTheme.maybeBrightnessOf(context));
    var contents = child;

    if (isSurfacePainted) {
      contents = ColoredBox(
        color: CupertinoDynamicColor.resolve(
          dialogColor ?? _kDialogColor,
          context,
        ),
        child: contents,
      );
...
class _TargetSelectionGestureRecognizer extends GestureRecognizer {
  ...
  void _updateDrag(Offset pointerPosition, {required bool fromPointerDown}) {
    final result = hitTest(pointerPosition);
...
class _CupertinoActionSheetState extends State<CupertinoActionSheet> {
  ...
  Widget? _buildContent(BuildContext context) {
    ...
    final textStyle = _kActionSheetContentStyle.copyWith(
      color: CupertinoDynamicColor.resolve(
  ...
  double _topPadding(BuildContext context) {
    ...
    const double viewPaddingData1 = 47;
    const double paddingRatioData1 = 1;
    const double viewPaddingData2 = 59;
    const paddingRatioData2 = 54.0 / 59.0;

    final currentViewPadding = MediaQuery.viewPaddingOf(context).top;

    final currentPaddingRatio = _lerp(
    ...
    final padding = (currentPaddingRatio * currentViewPadding).roundToDouble();
  ...
  Widget build(BuildContext context) {
    ...
    final actionSheetWidth = switch (MediaQuery.orientationOf(context)) {
...
class _CupertinoActionSheetActionState extends State<CupertinoActionSheetAction>
  ...
  Widget build(BuildContext context) {
    ...
    var style = _kActionSheetActionStyle.copyWith(
...
class _CupertinoAlertActionSection extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final dialogColor = CupertinoDynamicColor.resolve(
      this.dialogColor ?? _kDialogColor,
      context,
    );
    final dialogPressedColor = CupertinoDynamicColor.resolve(
...
class _AlertDialogButtonBackground extends StatefulWidget {
  ...
  @override
  _AlertDialogButtonBackgroundState createState() =>
...
/// A button typically used in a [CustomizableCupertinoAlertDialog].
///
/// See also:
///
///  * [CustomizableCupertinoAlertDialog], a dialog that informs the user about situations
...
class _CupertinoDialogActionState extends State<CupertinoDialogAction>
  ...
  @override
  Widget build(BuildContext context) {
    var style = _kCupertinoDialogActionStyle
        .copyWith(
          color: CupertinoDynamicColor.resolve(
...
class _AlertDialogActionsLayout extends MultiChildRenderObjectWidget {
  ...
  void updateRenderObject(
    ...
    _RenderAlertDialogActionsLayout renderObject,
  ) {
...
class _RenderAlertDialogActionsLayout extends RenderFlex {
  ...
  double computeMinIntrinsicHeight(double width) {
    ...
    final slotWidth = horizontalSlotWidthFor(overallWidth: width);
    double height = 0;
  ...
  Size computeDryLayout(covariant BoxConstraints constraints) {
    ...
    final overallWidth = constraints.maxWidth;
    if (!_useHorizontalLayout(overallWidth)) {
      return super.computeDryLayout(constraints);
    }

    final height = getMinIntrinsicHeight(overallWidth);
  ...
  void performLayout() {
    ...
    final slotWidth = horizontalSlotWidthFor(overallWidth: overallWidth);
    final height = getMinIntrinsicHeight(overallWidth);
    ...
    while (true) {
      slot.layout(
        BoxConstraints.tight(Size(slotWidth, height)),
        parentUsesSize: true,
      );
      (slot.parentData! as FlexParentData).offset = Offset(x, 0);
      x += slot.size.width;

      final divider = childAfter(slot);
      if (divider == null) {
        break;
      }
  ...
  bool _useHorizontalLayout(double overallWidth) {
    ...
    while (true) {
      ...
      final divider = childAfter(child);
      if (divider == null) {
        break;
      }
...
class _PriorityColumn extends MultiChildRenderObjectWidget {
  ...
  void updateRenderObject(
    BuildContext context,
    _RenderPriorityColumn renderObject,
  ) {
...
class _RenderPriorityColumn extends RenderFlex {
  ...
  void performLayout() {
    ...
    firstChild!.layout(
      BoxConstraints.tight(Size(width, topChildHeight)),
      parentUsesSize: true,
    );
    (firstChild!.parentData! as FlexParentData).offset = Offset.zero;

    lastChild!.layout(
      BoxConstraints.tight(Size(width, bottomChildHeight)),
      parentUsesSize: true,
    );
    (lastChild!.parentData! as FlexParentData).offset = Offset(
      0,
      topChildHeight,
    );
  }
  ...
  _TwoChildrenHeights _childrenHeights(double width, double maxHeight) {
    assert(childCount == 2);
    final topIntrinsic = firstChild!.getMinIntrinsicHeight(width);
    final bottomIntrinsic = lastChild!.getMinIntrinsicHeight(width);
    ...
    final double effectiveBottomMinHeight = math.min(
      _bottomMinHeight,
      bottomIntrinsic,
    );
